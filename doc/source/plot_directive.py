...
def run_code(plot_path, function_name, plot_code):
    ...
    if plot_code is not None:
        ...
        print "DEBUG: plot_directive.run_code: path:", path
        print "DEBUG: plot_directive.run_code: name:", fname
        stdout = sys.stdout
...
def _plot_directive(plot_path, basedir, function_name, plot_code, caption,
    ...
    print "DEBUG _plot_directive: rstdir", rstdir
    print "DEBUG _plot_directive: rstfile", rstfile
    print "DEBUG _plot_directive: outdir", outdir
    print "DEBUG _plot_directive: reldir", reldir
    print "DEBUG _plot_directive: linkdir", linkdir
    ...
    destdir = os.path.abspath(os.path.join(builder.outdir, outdir))
    if not os.path.exists(destdir):
        cbook.mkdirs(destdir)
    print "DEBUG _plot_directive: destdir", destdir
    ...
    if plot_code is None:
        print "DEBUG plot_directive: plot_path", os.path.join('source', plot_path),
        print "DEBUG plot_directive: plot_path", os.path.join(destdir, fname)
        try:
            shutil.copyfile(plot_path, os.path.join(destdir, fname))
        except IOError:
            shutil.copyfile(os.path.join('source', plot_path),
                            os.path.join(destdir, fname))

    if options.has_key('include-source'):
        if plot_code is None:
            lines.extend(
                ['.. include:: %s' % os.path.join(builder.srcdir, plot_path),
                 '    :literal:'])
...
def plot_directive(name, arguments, options, content, lineno,
    ...
    if len(arguments):
        ...
        print "DEBUG plot_directive dirname", os.path.dirname(plot_path)
        print "DEBUG plot_directove builder", setup.app.builder.srcdir
        print "DEBUG plot_directive: basedir", basedir
        print "DEBUG plot_directive: plotpath", plot_path
